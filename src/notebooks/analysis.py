...
app = marimo.App(width='full', app_title='Analysis: Sensitivity and Convergence')
...
def imports():
    ...
    from notebooks.utils.config import PARTICLE_SIZE_RANGES, RANDOM_SEED
    from notebooks.utils.simulations import (
        compute_clogging,
        compute_physics,
...
@app.cell
def run_convergence_analysis(simulate_nozzle_clogging, pd):
...
@app.cell
def plot_convergence(convergence_df, mo, np, plt):
    ...
    plt.tight_layout()
    mo.md('**Figure S1:** Convergence of key statistics with increasing sample size.')
    return (_fig,)
...
def run_sobol_analysis(
    compute_clogging, compute_physics, np, pd, sobol_analyze, sobol_sample
):
...
@app.cell
def plot_sobol_indices(Si, mo, np, plt):
    ...
    plt.tight_layout()
    mo.md(
        "**Figure S2:** Sobol' sensitivity indices. First-order (S1) measures direct effect; "
...
@app.cell
def display_sobol_table(Si, mo, np, pd):
...
def plot_polydisperse_comparison(comparison, mo, np, plt):
    ...
    plt.tight_layout()
    mo.md(
        f'**Figure S3:** Mono- vs polydisperse comparison. '
